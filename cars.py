...
# pdb.set_trace()
temp = None
cap = cv2.VideoCapture('cars_pass.mp4')
wheel_count = []

# flag to check if second tire has been found in the frame
tire = 0

# counter for which car number we are at
car = 1

# flag to check if its current car, is set to one a car is in frame and to zero again
# after there no car for a certain period in frame
flag = 0

# dictionary of  each wheel position at each frame
wheel_position = {}
...
while (cap.isOpened()):
    ...
    if ret:
        ...
        def current_speed(cur_wheel):
            ...
            start = s[0][0] * 1.0 #153.0
            end = e[0][0] * 1.0
            s_frame = s[1] * 1.0 #15.0
            e_frame = e[1] * 1.0
        ...
        # Function for template matching to remove wrongly detected circles
        def ssd(img1):
            try:
                tire1 = cv2.imread("crop_wheel.png", 0)
                """
                ...
                dist = cv2.morphologyEx(tire1, cv2.MORPH_CLOSE, kernel)
                dist1 = cv2.morphologyEx(img1, cv2.MORPH_CLOSE, kernel)
                dist = cv2.resize(dist, (150, 150), interpolation=cv2.INTER_CUBIC)
                dist1 = cv2.resize(dist1, (150, 150), interpolation=cv2.INTER_CUBIC)
                diff = dist - dist1
                diff = diff ** 2
                ssd = np.sum(diff) #217195
                return ssd
            except:
                pass
        ...
        for i in circles[0, :]:
            ...
            """ 
                 Assumptions: tires are in same line, they don't deviate much in vertical direction,
                 this way circles aren't detected outside tire path.
            """
            if (i[2] < 75 and i[2] > 53 and i[1] < 560 and i[1] > 500):
                ...
                # W is  contains circles associated with tires
